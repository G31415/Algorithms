...
int *find_maximum_subarray(int *a, int low, int high) // 找到最大子数组
{
    ...
    else
    {
        ...
        if (max_crossing[0] >= max_left[0] && max_crossing[0] >= max_right[0])
            return max_crossing;
        if (max_left[0] >= max_right[0] && max_left[0] >= max_crossing[0])
            return max_left;
        if (max_right[0] >= max_left[0] && max_right[0] >= max_crossing[0])
            return max_right;
    }
    return NULL;
}
